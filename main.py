...
import randomuser as ruser
import sys
sys.path.append('C:/Projects/finalproject')  #  경로
...
def view_allusers(limit, offset):
    conn = sqlite3.connect('user_data.db')
    c = conn.cursor()
...
def search_user_bynameoremail(search_term):
    conn = sqlite3.connect('user_data.db')
    c = conn.cursor()
...
def update_userbyemail(old_email, new_name, new_email, new_role):
    conn = sqlite3.connect('user_data.db')
    c = conn.cursor()
...
def main():
    if cookies.get('logged_in') == 'True':
        ...
        elif st.session_state.get('page') == "random_users":
            st.title("Admin - Random User")
        else:
    ...
    if cookies['logged_in'] == 'True':
        ...
        if st.sidebar.button("Random User"):
            st.session_state['page'] = "random_users"            
    ...
    if st.session_state['page'] == "view_users" and cookies.get('logged_in') == 'True':       
        ...
        if search_term:
            users = search_user_bynameoremail(search_term)
        else:
            users = view_allusers(limit=per_page, offset=offset)
        ...
        if users:
            ...
            for index, row in df.iterrows():
                ...
                if col6.button("Delete", key=f"delete_{row['Email']}_{index}"):
                    delete_userbyemail(row['Email'])
                    st.success(f"User {row['Email']} deleted successfully")
                    st.rerun()

           
            total_users = get_usercount()
    ...
    if 'edit_email' in st.session_state and st.session_state['page'] == "edit_user":
        ...
        search_result = search_user_bynameoremail(edit_user_email)
        ...
        if len(search_result) == 0:
            ...
            if st.button("Update User"):
                if new_email != user_to_edit[2] and len(search_user_bynameoremail(new_email)) > 0:
                    st.error(f"Email {new_email} is already in use. Please use a different email.")
                else:
                    update_userbyemail(edit_user_email, new_name, new_email, new_role)
    ...
    if st.session_state['page'] == "random_users":
        st.subheader("Generate Random Users")

        num_users = st.number_input("Number of users to generate", min_value=1, max_value=1000, value=10)
        if st.button("Generate"):
            user_infos = []
            for _ in range(num_users):
                name = ruser.get_rand_name()
                phone = ruser.get_rand_phone_number()
                member_id = ruser.get_rand_member_ID()
                user_infos.append([name, phone, member_id])

            columns_name = ['Full Name', 'Phone Number', 'User ID']
